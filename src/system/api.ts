...
import Window, { WindowOptions, WindowType } from "./core/wm/Window";
import { ADR } from "./adr";
...
export default function createLegacySystemApi(wm: WindowManager, fs: IFileSystem, auth: IAuthentication) {
	...
	console.log("Creating legacy System API...");
	return (legacySystemApi = {
		hash: (str: string) => {
			var h = 0;
		...
		UAC: {
			checkPrivilege: (required: XpUserPrivilege) => {
				const user = legacySystemApi.Auth.getCurrentUser();
				if (!user)
					return false;
				if (user.privilege === XpUserPrivilege.ADMIN)
					return true;
				if (required === XpUserPrivilege.USER && user.privilege === XpUserPrivilege.USER) return true;
				return false;
			},
			requestEscalation: (callback: (result: boolean) => void) => {
				if (legacySystemApi.UAC.checkPrivilege(XpUserPrivilege.ADMIN)) {
					callback(true);
					return;
				}
				...
				*/
				...
				for (const u in users)
					if (users[u]!.privilege === XpUserPrivilege.ADMIN)
						admins.push(users[u]!);

				const select = legacySystemApi.createElement({
					tag: 'select',
					style: {
						width: '100%',
						padding: '2px',
				...
				const pwdInput = legacySystemApi.createElement({ tag: 'input', type: 'password', style: { width: '100%', padding: '2px', border: '1px solid #7f9db9' }, placeholder: 'Password' });
				container.appendChild(pwdInput);

				const btnGroup = legacySystemApi.createElement({ tag: "div", style: { display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: 'auto' } });
				const okBtn = legacySystemApi.createElement({ tag: 'button', innerText: 'OK', className: 'xp-button', style: { padding: '2px 20px' } });
				const cancelBtn = legacySystemApi.createElement({ tag: 'button', innerText: 'Cancel', className: 'xp-button', style: { padding: '2px 20px' } });
				...
				okBtn.onclick = () => {
					...
					if (admin.passwordHash === legacySystemApi.hash(pwdInput.value)) {
						cleanup();
						callback(true);
					} else {
				...
				winId = legacySystemApi.createWindow({
				...
				const winEl = wm.getById(winId)?.element;
				if (winEl)
		...
		FS: {
			checkAccess: (path: string, operation: "read" | "write"): boolean => {
				...
				if (!stat) {
					...
					return legacySystemApi.FS.checkAccess(parentPath, "write");
				}
			...
			readFile: (path: string) => {
				if (legacySystemApi.FS.checkAccess(path, "read"))
			...
			writeFile: (path: string, content: string) => {
				if (legacySystemApi.FS.checkAccess(path, "write"))
		...
		exec: (path: string, args?: unknown): boolean => {
			...
			if (path.endsWith(".lnk")) {
				...
				if (stat && (stat as FSFileNode).isLink) {
					...
					try {
						...
						return legacySystemApi.exec(linkData.app, [linkData.args]);
					} catch (e) {
			...
			handleExtension: if (filename.includes(".")) {
				...
				const associations = legacySystemApi.Registry.get<Record<string, string>>("System/Associations");
		...
		getSCT: <T>() => {
			return legacySystemApi.Registry.get<T>("System");
		},
		setSCT: <T>(data: T) => {
			return legacySystemApi.Registry.set("System", data);
		},
		...
		getIcon: (path: string) => {
			const cache = legacySystemApi.getIconCache();
			if (cache[path])
				return cache[path];
			...
			cache[path] = iconUrl;
			legacySystemApi.setIconCache(cache);
			return iconUrl;
		},
		Registry: (() => {
		...
		addTrayIcon (options: ITrayIconOptions) {
			...
			return {
				showBalloon: (options: BalloonTipOptions) => {
					legacySystemApi.showBalloonTip(icon, options);
				},
			};
		},
		...
		showDialog: (options: IDialogOptions) => {
			...
			win = wm.create({
				...
				isDialog: true,
				content: container,
				type: options.modal ? WindowType.MODAL : (options.topmodal ? WindowType.TOPMODAL : WindowType.NORMAL),
		...
		showInstaller: (options: IInstallerOptions) => {
			...
			winId = legacySystemApi.createWindow({
				...
				isDialog: true,
				content: installer,
				type: options.modal ? WindowType.MODAL : WindowType.NORMAL,
			});
			return winId;
		}
	});
...
export interface ILegacySystemAPI {
	...
	Auth: {
		...
		logout(): void;
		getCurrentUser(): XpUser | undefined;
	};
	UAC: {
	...
	setWindowContent(id: number, content: string | HTMLElement): void;
	setWindowTitle(id: number, title: string): void;
	updateTaskbar(): void;
	addTrayIcon(options: ITrayIconOptions): {
		showBalloon(options: BalloonTipOptions): void;
	};
	...
	showInstaller(options: IInstallerOptions): void;
}

export enum XpUserPrivilege {
	ADMIN,
	USER,
	GUEST,
}
