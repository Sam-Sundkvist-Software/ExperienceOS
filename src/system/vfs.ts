...
export const VFS = (() => {
				...
				children: {
					...
					'StartMenu': {
						...
						children: {
							...
							"Debug Suite.lnk": { type: "file", content: JSON.stringify({ app: "debugProc", args: "" }), isLink: true },
						}
					},
					'Documents': {
						type: 'dir',
	...
	return {
		...
		stat: (path: string): FSFileNode | { type: "dir" } | null => {
			const node = resolvePath(path);
			if (!node)
				return null;
			if (node.type !== "file")
		...
		writeFile: (path: string, content: string, metadata?: FSNodeMetadata) => {
			const parts = path.split('/').filter(p => p.length > 0);
			...
			if (dirNode && dirNode.type === "dir") {
				dirNode.children[fileName] = {
					type: 'file',
					content: content,
					metadata: metadata || (dirNode.children[fileName] && dirNode.children[fileName].type === "file" ? dirNode.children[fileName].metadata : undefined)
				};
				return true;
			}
			return false;
		...
		walk: (path: string, callback: (p: string, n: FSNode) => void) => {
			const node = resolvePath(path);
			if (!node) return;
			
			const traverse = (n: FSNode, p: string) => {
				callback(p, n);
				if (n.type === 'dir') {
		...
		move: (oldPath: string, newDirPath: string) => {
			const parts = oldPath.split('/').filter(p => p.length > 0);
			const fileName = parts.pop();
			if (!fileName)
				return;
