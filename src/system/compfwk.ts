...
export function createElement<T extends keyof HTMLElementTagNameMap>(options: CreateElementOptions<T>) {
	...
	if (options.tooltip) {
		legacySystemApi.showTooltip(el, typeof options.tooltip === 'string' ? { text: options.tooltip } : options.tooltip);
	}
...
export const FCCF = (() => {
	const registry = () => legacySystemApi.Registry;
	...
	return {
		...
		Window: (options: {
			...
			return legacySystemApi.createWindow(winOptions);
		}
	};
})();
